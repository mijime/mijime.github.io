...
2. Buchheim の `firstWalk` → `secondWalk` で各ノードの prelim `x` と `y` (深さ) を決定
3. **ルート直属の子を `startAngle + i × 2π / N` に再配置** (mindmap 慣例の均等配置)
4. ツリー全体の `x` を `[startAngle, startAngle + 2π]` に線形スケール
5. 極座標に変換: `radius = depth × ringDistance`、`angle = x`
6. 直交座標を返す: `(cos(angle) × radius, sin(angle) × radius)`
...
### Step 3: ルート直属の子の均等配置 (mindmap 慣例)

d3 の second walk 完了後、**ルート直属の子**は prelim `x` ではなく `startAngle + i × 2π / N` (`i` = 子のインデックス、`N` = ルート直属の子数) に上書き配置する。これは mindmap の慣例 (「ルート子は円周上に均等に配置」「ひとり子は startAngle 方向」) を満たすための特殊ケース。

上書き時の処理:
1. 各ルート直属の子 `i` について `targetX = startAngle + i × 2π / N` を計算
2. d3 の second walk 結果 `currentX` との差分 `offset = targetX - currentX` を計算
3. その子と全子孫の `x` に `offset` を加算 (定数シフトなので Buchheim の重なりゼロ不変条件は保存される)

これにより:
- 1 個の子: `targetX = startAngle` (UP) に配置
- 2 個の子: `startAngle`, `startAngle + π` (180° 反対)
- 3 個の子: `startAngle`, `startAngle + 2π/3`, `startAngle + 4π/3` (120° 等間隔)
- 4 個の子: 12, 3, 6, 9 時方向 (90° 等間隔)

d3 のアルゴリズムはこの特殊ケースを行わない (純粋な Buchheim ではルート子も prelim ベースで配置され、均等にならない)。mindmap アプリとして「ルート子の均等配置」を優先するため、この特殊ケースを導入する。

### Step 4: prelim x の正規化

Step 3 完了後、ツリー全体の `x` 値域を `[startAngle, startAngle + 2π]` に線形スケールする:
```
minX = min(node.x for node in tree)
maxX = max(node.x for node in tree)
scaledX = startAngle + (x - minX) × (2π / (maxX - minX))
```
ルート直属の子の特殊ケースで既に `startAngle` から始まる角度範囲に配置されているため、スケール後の範囲は概ね `[startAngle, startAngle + 2π]` になる (単一ノードの場合はそのまま)。

### Step 5: 極座標 → 直交座標

各ノードの最終位置:
```
radius = depth × ringDistance
angle  = scaledX                  // [startAngle, startAngle + 2π]
x      = cos(angle) × radius
y      = sin(angle) × radius
```
...
- ルート直属でない兄弟は distinct angle に配置される (深さ比例 separation の効果)
...
- **`src/layout/radial.ts`**: 全面書き換え。Buchheim の `firstWalk` / `secondWalk` / `apportion` / `executeShifts` / `nextLeft` / `nextRight` / `moveSubtree` を d3-hierarchy `src/tree.js` から等価移植。`TreeNode` 内部クラス新設。`radialSeparation` 関数を追加。**Step 3 (ルート直属の子の均等配置)** を second walk 後に適用。prelim 正規化 + 極座標変換を追加。`ringFactor` 削除。`singleChildAngleInherit` 削除
